...
class TemporalPoints(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.weights = torch.nn.Parameter(self._weights_from_bones(joints,
            bones,
            canonical_pcd,
            add_noise=True,
            noise_var=0,
            soft_weights=True,
            add_zero_weight=True), requires_grad=True)
        self.forward_warp = PointWarper(canonical_pcd=canonical_pcd,
            t_dim=self.forward_warp_t_dim,
            joints=joints,
            bones=bones,
            over_parameterized_rot=over_parameterized_rot)
        ...
        self.nn_distance = torch.sqrt(((self.canonical_pcd[:, None, :] - self.canonical_pcd[self.nn_i,
                                                                         :]) ** 2).sum(-1) + self.eps)  # distance to nearest neighbour per point
        ...
        if pose_embedding_dim > 0:
            ...
            self.pose_embedding_net = torch.nn.Sequential(
                ...
                *[
                    torch.nn.Sequential(torch.nn.Linear(input_pose_embedding // 2, input_pose_embedding // 2),
    ...
    def reinitialise_weights(self):
        self.weights.data = self._weights_from_bones(self.joints,
            self.bones,
            self.canonical_pcd,
            add_noise=True,
            noise_var=0,
            soft_weights=True,
            add_zero_weight=True)
    ...
    def dist_batch(self, p, a, b):
        ...
        res[within_mask] += torch.norm(
            p[0][within_mask[1]] - (
                a[within_mask[0]] + (ps[within_mask] / l2[within_mask[0]]).unsqueeze(-1) * s[within_mask[0]]),
            dim=-1)

        return res

    def _weights_from_bones(
        self,
        joints,
        bones,
        pcd,
    ...
    def simplify_skeleton(
        ...
        five_percent_heuristic=False,
        visualise_canonical=False
    ):
        ...
        if five_percent_heuristic:
            ...
            # Avg heuristic
            deg_stds = torch.rad2deg((
                rot_angles ** 2).mean(dim=0))  # How much it differs from the canonical pose (zero rad)
        ...
        bones = self.bones
        new_joints, new_bones, merging_rules, joints_to_keep, rotations_to_keep, rotation_switch_mask, sibling_transfer_rules = merge_joints(
            joints,
            bones,
            prune_bones,
            rotation_similarity_mat,
            convert_merging_rules=False)
    ...
    def aggregate_pts(
        ...
        with torch.profiler.record_function("sample_ray"):
            ...
            if calc_min_max:
                xyz_min, xyz_max = torch.min(t_hat_pcd, dim=0)[0] - query_radius, torch.max(t_hat_pcd, dim=0)[
                    0] + query_radius
            else:
    ...
    def forward(
        self, t, render_depth=False, render_kwargs=None, query_radius=0.01,
...
def get_training_rays_in_maskcache_sampling(
    ...
    **kwargs
    ...
    N = sum(
        im.shape[0] * im.shape[1] for im in rgb_tr_ori)  # N = len(i_train) * rgb_tr_ori.shape[1] * rgb_tr_ori.shape[2]
    ...
    top = torch.tensor(0)
    for c2w, img, mask_tr_ori, (H, W), K, time_one, PI_iter in zip(
        train_poses[img_to_cam], rgb_tr_ori, masks_tr_ori, HW, Ks[img_to_cam], times, img_to_cam):
