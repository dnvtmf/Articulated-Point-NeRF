...
do
    ...
    then
      screen -S gpu${gpu_id} -p 0 -X stuff \
        "python run.py --config configs/zju/${scene}.py --i_print 1000 --render_video --render_pcd ^M"
    fi
    screen -S gpu${gpu_id} -p 0 -X stuff "python test.py --config configs/zju/${scene}.py --num_fps=1000 ^M"
done
